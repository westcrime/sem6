...
ipcMain.on(e[0], async (event, args) => {
    console.log(args);
    ...
    ipcMain.on(e[1], async (event, args) => {
        console.log(args);
        try {
...
ipcMain.on(e[2], async (event, args) => {
    console.log(args);
    const response = await db.collection('users').get();
    let responseArr = [];
    response.forEach(doc => {
...
ipcMain.on(e[3], async (event, args) => {
    const { email, password } = args;
    const response = await db.collection('users').get();
    let responseArr = [];
    response.forEach(doc => {
    ...
    ipcMain.on(e[4], async (event, args) => {
        try {
...
ipcMain.on(e[5], async (event, args) => {
    let response = {};
    try {
        const verified = jwt.verify(args.token, jwtSecretKey);
        if (verified) {
...
ipcMain.on(e[6], async (event, args) => {
    let response = {};
    try {
        const verified = jwt.verify(args.token, jwtSecretKey);
        if (verified) {
